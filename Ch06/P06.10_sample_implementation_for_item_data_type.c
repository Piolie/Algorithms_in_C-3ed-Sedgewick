...
the type of the data directly with double, use explicit floating-point
options in scanf and printf, and so forth. We include the interface
file Item.h so that we will discover at compile time any discrepancies
...
int ITEMscan(double *x) { return scanf("%f", x); }
