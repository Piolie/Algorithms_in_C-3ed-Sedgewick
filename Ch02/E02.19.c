...
double pow_3_2(unsigned int n) { return pow(n, 3.0 / 2.0); }

double pow_5_4(unsigned int n) { return pow(n, 5.0 / 4.0); }
...
double twoNH_N(unsigned int n) { return 2 * n * H_N(n); }

double lgN(unsigned int n) { return log(n) / log(2); }

double NlgNlglgN(unsigned int n) { return n * lgN(n) * lgN(lgN(n)); }

double N2lgN(unsigned int n) { return n * n * lgN(n); }
